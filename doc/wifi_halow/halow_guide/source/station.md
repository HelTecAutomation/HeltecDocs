...
- [HT-HR01](https://docs.heltec.org/en/wifi_halow/ht-hr01/index.html#get-started)
...
|  Mode   | Description  |
|  ----  | :----- |
| None  | In None traffic mode, non-HaLow and HaLow networks are isolated. This device will use a static IP address and run a DHCP server on the non-HaLow interface |
| Bridge  | In Bridged traffic mode, non-HaLow devices obtain IPs from your HaLow link |
| Extender  | In Extender traffic mode, non-HaLow devices obtain IPs from the DHCP server on this device and this device uses NAT to forward IP traffic |
